...
window.nkXRInputSource =
{
    ...
    GetXRHand: function (uid, d)
    {
        ...
        //for (var jointName of hand.keys()) {
        //    var joint = hand.get(jointName);
        //    var jid = nkJSObject.GetUid(joint);
        //    if (jid === -1)
        //        jid = nkJSObject.RegisterObject(joint);
        //    //Module.HEAP32[pt >> 2] = jid;
        //    //pt += 4;
        //}

        var hid = nkJSObject.GetUid(hand);
        if (hid !== -1)
            return hid;

        return nkJSObject.RegisterObject(hand);
    },
};

window.nkXRHand =
{
    GetJoints: function (uid, d) {
        var hand = nkJSObject.GetObject(uid);
        var pt = Module.HEAP32[(d + 0) >> 2];
    ...
    GetFrame: function (uid, d) {

        var hand = nkJSObject.GetObject(uid);

        var frid = Module.HEAP32[(d + 0) >> 2];
        var bsid = Module.HEAP32[(d + 4) >> 2];
        var fr = nkJSObject.GetObject(frid);
        var bs = nkJSObject.GetObject(bsid);

        var pt = Module.HEAP32[(d + 8) >> 2];

        var jointSpaces = Array.from(hand.values());
        var r = new Float32Array(jointSpaces.length);
        fr.fillJointRadii(jointSpaces, r);
        for (var i = 0; i < r.length; i++) {
            Module.HEAPF32[pt >> 2] = r[i];
            pt += 4;
        }

        var p = new Float32Array(jointSpaces.length * 16);
        fr.fillPoses(jointSpaces, bs, p);
        for (var i = 0; i < p.length; i++) {
            Module.HEAPF32[pt >> 2] = p[i];
            pt += 4;
        }
    },
};
...
//window.nkXRHands = {
//    GetHandJoints: function (uid, d) {
//        var hand = nkJSObject.GetObject(uid);
//        var joints = hand.joints;

//        var jointUids = [];
//        for (var joint of joints) {
//            var jointUid = nkJSObject.GetUid(joint);
//            if (jointUid === -1) {
//                jointUid = nkJSObject.RegisterObject(joint);
//            }
//            jointUids.push(jointUid);
//        }
...
//        var joint = nkJSObject.GetObject(uid);
//        var pt = Module.HEAP32[(d + 0) >> 2];
...
//            Module.HEAPF32[(pt + 0) >> 2] = orientation.x;
//            Module.HEAPF32[(pt + 4) >> 2] = orientation.y;
//            Module.HEAPF32[(pt + 8) >> 2] = orientation.z;
//            Module.HEAPF32[(pt + 12) >> 2] = orientation.w;

//            Module.HEAPF32[(pt + 16) >> 2] = position.x;
//            Module.HEAPF32[(pt + 20) >> 2] = position.y;
//            Module.HEAPF32[(pt + 24) >> 2] = position.z;
//        }
//    },

//    GetJointRadius: function (uid, d) {
//        var joint = nkJSObject.GetObject(uid);
//        return joint.radius;
//    },
//};
